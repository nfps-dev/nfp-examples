...
use crate::nfp::{
    ...
    add_tags_to_package_version, PackageVersion, get_package_version_by_tag, get_package_version_by_index, get_package_versions_info, 
    RawData, PREFIX_STORAGE_OWNER, PREFIX_STORAGE_TOKEN, PREFIX_STORAGE_GLOBAL, store_package_version,};
...
use crate::token::{Metadata, Token, StoredMetadata};
...
pub fn instantiate(
    ...
    let config = Config {
        ...
        minter_may_put_token_storage: init_config.minter_may_put_token_storage.unwrap_or(false),
        minter_may_put_global_storage: init_config.minter_may_put_global_storage.unwrap_or(false),
    };
...
pub fn execute(deps: DepsMut, env: Env, info: MessageInfo, msg: ExecuteMsg) -> StdResult<Response> {
    ...
    let response = match msg {
        ...
        // NFP
        ExecuteMsg::StorageOwnerPut { data, owner, token_id, .. } => {
            storage_owner_put(deps, &info.sender, &config, data, owner, token_id)
        },
        ExecuteMsg::ApproveOwnerDelegate { address, .. } => {
            approve_owner_delegate(deps, &info.sender, address)
        },
        ExecuteMsg::ApproveTokenDelegate { address, token_ids, .. } => {
            approve_token_delegate(deps, &info.sender, address, token_ids)
        },
        ExecuteMsg::RevokeDelegate { address, .. } => {
            revoke_delegate(deps, &info.sender, address)
        },
        ExecuteMsg::RevokeAllDelegates { .. } => {
            revoke_all_delegates(deps, &info.sender)
        },
        ExecuteMsg::StorageTokenPut { data, token_id, .. } => {
            if !config.minter_may_put_token_storage {
                return Err(StdError::generic_err("Unauthorized"));
            }
            storage_token_put(deps, &info.sender, data, token_id)
        },
        ExecuteMsg::StorageGlobalPut { data, .. } => {
            if !config.minter_may_put_global_storage {
                return Err(StdError::generic_err("Unauthorized"));
            }
            storage_global_put(deps, &info.sender, data)
        },
        ExecuteMsg::UploadPackageVersion { 
            package_id, 
            data, 
            tags, 
            metadata,
        ...
        ExecuteMsg::AddPackageTags { package_id, index, tags, .. } => add_package_tags(
            deps,
            &info.sender,
            &config,
            ContractStatus::StopTransactions.to_u8(),
...
/// Returns StdResult<Response>
///
/// puts a set of key-value pairs in owner storage
///
/// # Arguments
///
/// * `deps` - mutable reference to Extern containing all the contract's external dependencies
/// * `sender` - addr of sender
/// * `data` - vector of key-value pairs to put in owner storage
/// * `owner` - optional owner (in case called by delegate)
/// * `token_id` - optional token id (in case called by delegate of current owner of token id)
pub fn storage_owner_put(
    deps: DepsMut,
    sender: &Addr,
    config: &Config,
    ...
    if owner.is_some() && token_id.is_some() {
        return Err(StdError::generic_err("storage_owner_put cannot have `owner` and `token_id` both set"))
    }

    let not_authorized_msg = "Not authorized to set storage of owner";

    let owner_raw: CanonicalAddr;
    if let Some(token_id) = token_id {    
        ...
        owner_raw = token.owner;
    } else if let Some(owner) = owner {
        owner_raw = deps.api.addr_canonicalize(&owner)?;
        if !ANY_DELEGATES
            .add_suffix(owner_raw.as_slice())
            .contains(deps.storage, &sender) {
                return Err(StdError::generic_err(not_authorized_msg));
        }
    } else {
    ...
    data.into_iter().try_for_each(|pair| {
        if pair.key.len() == 0 {
            return Err(StdError::generic_err("Empty key string in key value pair is invalid"));
        }

        let mut key_value_store = PrefixedStorage::multilevel(
            deps.storage, 
            &[PREFIX_STORAGE_OWNER, owner_raw.as_slice()]
        );

        if pair.value.is_some() {
            let value = pair.value.unwrap();
            save(&mut key_value_store, pair.key.as_bytes(), &value)?;
            //println!("key_value_store: {:?}", [PREFIX_STORAGE_OWNER, sender.as_slice()]);
            //println!("pair.key.as_bytes(): {:?}", pair.key.as_bytes());
            //println!("value: {:?}", value);
        } else {
            remove(&mut key_value_store, pair.key.as_bytes());
        }
        Ok(())
    })?;

    Ok(Response::new().set_data(to_binary(&ExecuteAnswer::StorageOwnerPut { status: Success })?))
}

/// Returns StdResult<Response>
///
/// sets approval for a specified address to set owner storage for ANY token they own
///
/// # Arguments
///
/// * `deps` - mutable reference to Extern containing all the contract's external dependencies
/// * `sender` - addr of sender (owner)
/// * `address` - addr of the account being delegated approval
fn approve_owner_delegate(
    deps: DepsMut,
    sender: &Addr,
    address: String,
) -> StdResult<Response> {
    ...
    add_any_delegate(deps.storage, sender_raw, address_raw)?;

    Ok(Response::new().set_data(to_binary(&ExecuteAnswer::ApproveOwnerDelegate { status: Success })?))
}

/// Returns StdResult<Response>
///
/// sets approval for a specified address to set owner storage if owner has a specific token
///
/// # Arguments
///
/// * `deps` - mutable reference to Extern containing all the contract's external dependencies
...
fn approve_token_delegate(
    ...
    add_token_delegate(deps.storage, sender_raw, address_raw, token_ids)?;

    Ok(Response::new().set_data(to_binary(&ExecuteAnswer::ApproveTokenDelegate { status: Success })?))
}

/// Returns StdResult<Response>
///
/// revokes approval for a specified address to set owner storage for ANY token the sender owns
///
/// # Arguments
///
/// * `deps` - mutable reference to Extern containing all the contract's external dependencies
...
fn revoke_delegate(
    ...
    remove_any_delegate(deps.storage, sender_raw.clone(), address_raw.clone())?;
    remove_token_delegate(deps.storage, sender_raw, address_raw)?;

    Ok(Response::new().set_data(to_binary(&ExecuteAnswer::RevokeDelegate { status: Success })?))
}

/// Returns StdResult<Response>
///
/// revokes all delegations to set owner storage for ANY token the sender owns
///
/// # Arguments
///
/// * `deps` - mutable reference to Extern containing all the contract's external dependencies
/// * `sender` - addr of sender (owner)
/// * `address` - addr of the account being delegated approval
fn revoke_all_delegates(
    deps: DepsMut,
    sender: &Addr,
) -> StdResult<Response> {
    let sender_raw = deps.api.addr_canonicalize(sender.as_str())?;

    remove_all_any_delegates(deps.storage, sender_raw.clone())?;
    remove_all_token_delegates(deps.storage, sender_raw)?;

    Ok(Response::new().set_data(to_binary(&ExecuteAnswer::RevokeAllDelegates { status: Success })?))
}

/// Returns StdResult<Response>
///
/// puts a set of key-value pairs in token storage
///
/// # Arguments
///
/// * `deps` - mutable reference to Extern containing all the contract's external dependencies
/// * `sender` - addr of sender
/// * `data` - vector of key-value pairs to put in token storage
/// * `token_id` - token id
fn storage_token_put(
    deps: DepsMut,
    sender: &Addr,
    data: Vec<KeyValuePair>,
    token_id: String
    ...
    if !minters.contains(&sender_raw) {
        return Err(StdError::generic_err(
            "Only designated minters can upload package versions",
        ));
    }
    //if config.admin != sender_raw {
    //    return Err(StdError::generic_err(
    ...
    Ok(Response::new().set_data(to_binary(&ExecuteAnswer::StorageTokenPut { status: Success })?))
}

/// Returns StdResult<Response>
///
/// puts a set of key-value pairs in global storage
///
/// # Arguments
///
/// * `deps` - mutable reference to Extern containing all the contract's external dependencies
/// * `sender` - addr of sender 
/// * `config` - config 
/// * `data` - vector of key-value pairs to put in global storage
fn storage_global_put(
    deps: DepsMut,
    sender: &Addr,
    data: Vec<KeyValuePair>, 
) -> StdResult<Response> {
    ...
    data.into_iter().try_for_each(|pair| {
        ...
        let mut key_value_store = PrefixedStorage::new(
            deps.storage, 
            PREFIX_STORAGE_GLOBAL
        );
    ...
    Ok(Response::new().set_data(to_binary(&ExecuteAnswer::StorageGlobalPut { status: Success })?))
}

/// Returns StdResult<Response>
///
/// stores a new version of a package
///
/// # Arguments
///
/// * `deps` - mutable reference to Extern containing all the contract's external dependencies
...
/// * `data` - data for the script as RawData
/// * `tags` - optional vec of tags
...
fn upload_package_version(
    ...
    let valid_access: Vec<String> = vec![ACCESS_PUBLIC_STRING, ACCESS_OWNERS_STRING, ACCESS_CLEARED_STRING]
        .into_iter()
        .map(|s| String::from(s))
        .collect();
    if !valid_access.contains(&access) {
        return Err(StdError::generic_err(format!("{} is not a valid access option", access)));
    }
    //if config.admin != sender_raw {
    //    return Err(StdError::generic_err(
    //        "This is an admin command and can only be run from the admin address",
    ...
    let index = store_package_version(deps.storage, package_id, data, tags, metadata, access)?;
    Ok(Response::new().set_data(to_binary(&ExecuteAnswer::UploadPackageVersion { index })?))
}
...
fn add_package_tags(
    ...
    add_tags_to_package_version(deps.storage, package_id, index, tags)?;

    Ok(Response::new().set_data(to_binary(&ExecuteAnswer::AddPackageTags { status: Success })?))
}
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    let response = match msg {
        ...
        // NFP
        QueryMsg::StorageOwnerGet { 
            keys, 
            viewer, 
        } => {
            ViewingKey::check(deps.storage, &viewer.address, &viewer.viewing_key)?;
            let viewer = Some(viewer);
            query_storage_owner_get(deps, viewer, None, keys)
        }
        QueryMsg::StorageTokenGet {
            keys,
            token_id,
            viewer,
        } => {
            ViewingKey::check(deps.storage, &viewer.address, &viewer.viewing_key)?;
            let viewer = Some(viewer);
            query_storage_token_get(deps, &env.block, viewer, None, keys, &token_id)
        }
        QueryMsg::StorageGlobalGet { 
            keys,  
        } => {
            query_storage_global_get(deps, keys)
        }
        QueryMsg::OwnerDelegateApprovals { 
            page, 
            page_size, 
            viewer, 
        } => {
            ViewingKey::check(deps.storage, &viewer.address, &viewer.viewing_key)?;
            let viewer = Some(viewer);
            query_owner_delegate_approvals(deps, viewer, None, page, page_size)
        }
        QueryMsg::TokenDelegateApprovals {
            token_id,
            viewer,
        } => {
            ViewingKey::check(deps.storage, &viewer.address, &viewer.viewing_key)?;
            let viewer = Some(viewer);
            query_token_delegate_approvals(deps, viewer, None, token_id)
        }
        QueryMsg::PackageVersion { 
            package_id, 
            tag, 
            index, 
            token_id, 
            ...
            let viewer_with_address: Option<ViewerInfo>;
            if let Some(viewer) = viewer {
                if let Some(address) = viewer.address {
                    viewer_with_address = Some(ViewerInfo {
                        address,
                        viewing_key: viewer.viewing_key,
                    });
                } else {
                    viewer_with_address = Some(ViewerInfo {
                        address: "".to_string(),
                ...
                viewer_with_address = None;
            }
            
            query_package_version(deps, &env.block, viewer_with_address, None, &token_id, package_id, tag, index)
        }
        QueryMsg::PackageInfo { package_id, page, page_size, viewer} => {
...
pub fn permit_queries(
    ...
    match query {
        ...
        // NFP
        QueryWithPermit::StorageOwnerGet { keys } => {
            query_storage_owner_get(deps, None, Some(querier), keys)
        }
        QueryWithPermit::StorageTokenGet { keys, token_id } => {
            query_storage_token_get(deps, &env.block, None, Some(querier), keys, &token_id)
        }
        QueryWithPermit::OwnerDelegateApprovals { page, page_size } => {
            query_owner_delegate_approvals(deps, None, Some(querier), page, page_size)
        }
        QueryWithPermit::TokenDelegateApprovals { token_id } => {
            query_token_delegate_approvals(deps, None, Some(querier), token_id)
        }
        QueryWithPermit::PackageVersion { package_id, tag, index, token_id } => {
            query_package_version(deps, &env.block, None, Some(querier), &token_id, package_id, tag, index)
        }
...
/// Returns values for a set of keys in owner storage
///
/// # Arguments
///
/// * `deps` - a reference to Extern containing all the contract's external dependencies
/// * `viewer` - optional address and key making an authenticated query request
/// * `from_permit` - address derived from an Owner permit, if applicable
/// * `keys` - array of keys to get from owner storage
pub fn query_storage_owner_get(
    deps: Deps,
    viewer: Option<ViewerInfo>,
    from_permit: Option<CanonicalAddr>,
    keys: Vec<String>,
    ...
    to_binary(&QueryAnswer::StorageOwnerGet { data })
}

/// Returns values for a set of keys in token storage
///
/// # Arguments
///
/// * `deps` - a reference to Extern containing all the contract's external dependencies
/// * `viewer` - optional address and key making an authenticated query request
/// * `from_permit` - address derived from an Owner permit, if applicable
/// * `keys` - array of keys to get from token storage
/// * `token_id` - string slice of token id
pub fn query_storage_token_get(
    deps: Deps,
    block: &BlockInfo,
    viewer: Option<ViewerInfo>,
    from_permit: Option<CanonicalAddr>,
    ...
    if check_permission(
        ...
        return Err(StdError::generic_err("You do not have permission to get token storage"));
    }
    ...
    to_binary(&QueryAnswer::StorageTokenGet { data })
}

/// Returns values for a set of keys in global storage
///
/// # Arguments
///
/// * `deps` - a reference to Extern containing all the contract's external dependencies
/// * `keys` - array of keys to get from owner storage
pub fn query_storage_global_get(
    deps: Deps,
    keys: Vec<String>,
) -> StdResult<Binary> {
    let key_value_store = ReadonlyPrefixedStorage::new(
        deps.storage, 
        PREFIX_STORAGE_GLOBAL
    );
    ...
    to_binary(&QueryAnswer::StorageGlobalGet { data })
}

pub fn query_owner_delegate_approvals(
    deps: Deps,
    viewer: Option<ViewerInfo>,
    from_permit: Option<CanonicalAddr>,
    page: Option<u32>,
    ...
    let owner = get_querier(deps, viewer, from_permit)?.ok_or_else(|| {
        StdError::generic_err("This is being called incorrectly if there is no querier address")
    })?;

    let page = page.unwrap_or(0_u32);
    let page_size = page_size.unwrap_or(10_u32);

    let addresses = ANY_DELEGATES
        .add_suffix(owner.as_slice())
        .paging(deps.storage, page, page_size)?
        .into_iter()
        .map(|address| 
            deps.api.addr_humanize(&address).and_then(|addr| Ok(addr.into_string()))
        )
        .collect::<Result<Vec<String>,_>>()?;

    to_binary(&QueryAnswer::OwnerDelegateApprovals { addresses })
}

pub fn query_token_delegate_approvals(
    deps: Deps,
    viewer: Option<ViewerInfo>,
    from_permit: Option<CanonicalAddr>,
    token_id: String,
    ...
    // check if viewer is owner of token
    let (token, _) = get_token(deps.storage, token_id.as_str(), None)?;
    ...
    //    prep_info.owner_is_public,
    //).is_err() {
    //    return Err(StdError::generic_err("You do not have permission to get token delegate approvals"));
    //}
    ...
    if let Some(addresses) = addresses {
        let addresses = addresses
            ...
            .collect::<Result<Vec<String>,_>>()?;
        to_binary(&QueryAnswer::TokenDelegateApprovals { addresses })
    } else {
        to_binary(&QueryAnswer::TokenDelegateApprovals { addresses: vec![] })
    }
}

pub fn query_package_version(
    deps: Deps,
    block: &BlockInfo,
    viewer: Option<ViewerInfo>,
    from_permit: Option<CanonicalAddr>,
    ...
    if tag.is_some() && index.is_some() {
        return Err(StdError::generic_err("Must have only one of `tag` or `index`"));
    }
    if tag.is_none() && index.is_none() {
        return Err(StdError::generic_err("Must include either `tag` or `index`"));
    }

    let package: Option<PackageVersion>;
    if let Some(tag) = tag {
        package = get_package_version_by_tag(deps.storage, package_id.clone(), tag)?;
    } else {
        let index = index.unwrap();
        package = get_package_version_by_index(deps.storage, package_id.clone(), index).ok();
    }

    let no_permission_err = "You do not have permission to view package version";
    if package.is_some() {
        ...
        if unwrapped_package.access != ACCESS_PUBLIC_STRING {
            ...
            if viewer.is_some() {
                ...
                if unwrapped_viewer.address == "" { 
                    // for packages we want to assume owner is querying, if not supplied
                    //   must still provide the correct viewing key
                    let err_msg = format!(
                        "You are not authorized to perform this action on token {}",
                        token_id
                    );
                    ...
                    viewer = Some(ViewerInfo { 
                        address: deps.api.addr_humanize(&token.owner)?.into_string(), 
                        viewing_key: unwrapped_viewer.viewing_key, 
                    });
                }
...
pub fn query_package_info(
    deps: Deps,
    package_id: String,
    page: Option<u32>,
    page_size: Option<u32>,
    viewer: Option<ViewerInfo>,
    querier: Option<CanonicalAddr>,
) -> StdResult<Binary> {
    let page = page.unwrap_or(0_u32);
    let page_size = page_size.unwrap_or(5_u32);

    let (mut info, mut version_count) =  get_package_versions_info(deps.storage, package_id.clone(), page, page_size)?;
    ...
    if info.len() > 0 {
        ...
        if access != ACCESS_PUBLIC_STRING {
            ...
            if access == ACCESS_OWNERS_STRING && inventory.cnt == 0 {
                // if it is owners access and they do not own any tokens, return no info
                info = vec![];
                version_count = 0;
            } else if access == ACCESS_CLEARED_STRING {
                let mut owns_cleared_token = false;
                let tokens = Vec::from_iter(inventory.to_set(deps.storage)?);

                for i in 0..tokens.len() {
                    let map2id = ReadonlyPrefixedStorage::new(deps.storage, PREFIX_MAP_TO_ID);
                    let may_id = may_load::<String>(&map2id, &(i as u32).to_le_bytes())?;

                    if let Some(id) = may_id {
                        let key_value_store = ReadonlyPrefixedStorage::multilevel(
                            deps.storage, 
                            &[PREFIX_STORAGE_TOKEN, id.as_bytes()]
                        );
                        let may_value = may_load::<String>(&key_value_store, &KEY_CLEARED_PACKAGES.as_bytes()).unwrap_or(None);
                        if let Some(value) = may_value {
                            let cleared_packages = Json::deserialize::<Vec<String>>(&value.as_bytes())?;
                            if cleared_packages.contains(&package_id) {
                                owns_cleared_token = true;
                                break;
                            }
                        }
                    }
                ...
                if !owns_cleared_token {
                    // if it is cleared access and they do not own any cleared tokens, return no info
                    info = vec![];
                    version_count = 0;
                }
            }
    ...
    to_binary(&QueryAnswer::PackageInfo { info, version_count })
}
...
pub fn dossier_list(
    ...
    for id in token_ids.into_iter() {
        ...
        let stored_public_metadata: Option<StoredMetadata> = may_load(&pub_store, &token_key)?;
        let public_metadata = stored_public_metadata.map(|meta| meta.into_humanized().unwrap());
        ...
        let private_metadata = if let Err(err) = check_perm_core(
            ...
            let stored_priv_meta: Option<StoredMetadata> = may_load(&priv_store, &token_key)?;
            let priv_meta = stored_priv_meta.map(|meta| meta.into_humanized().unwrap());
            priv_meta
        };
